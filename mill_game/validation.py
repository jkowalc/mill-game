    ...
    if board.allow_center_position:
        if not (0 <= rect <= rect_num):
            raise InvalidPawnPositionError(
                f"Pawn rectangle must be between 0 and {rect_num}"
            )
    else:
        if not (1 <= rect <= rect_num):
            raise InvalidPawnPositionError(
                f"Pawn rectangle must be between 1 and {rect_num}"
            )


def validate_initial_board(
    rect_num: int, initial_board_repr: list[list[PlayerColor | None]]
):
    if not len(initial_board_repr) == rect_num + 1:
        raise InvalidInitialBoardError(
            "Number of rectangles in board must be the same as declared"
        )
    for i, rect in enumerate(initial_board_repr):
        if i == 0 and len(rect) != 1:
            ...
            raise InvalidInitialBoardError(
                "Rectangles bigger than zero must contain 8 positions"
            )
